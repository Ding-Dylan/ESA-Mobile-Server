        ...
        oIpt.on('change', function (oEvent) {
            ...
            BaseUpload.uploadFile(oFile, {
                ...
                progress: function (nProgress) {
                    if (!oPopup) {
                        oPopup = new Popup({
                            content: '<div class="js-progress" style="text-align:center;font-size:14px;padding:20px 0;">Uploading:' + nProgress + '%<div>',
                            hasNoHeader: true
                        });
                    }
                    oPopup.getEl().find('div.js-progress').html('Uploading:' + nProgress + '%');
